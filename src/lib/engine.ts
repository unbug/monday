...
import { streamDeepSeek, type DeepSeekModel } from './deepSeekApi'
import {
  CreateMLCEngine,
...
export interface ProviderStreamOptions {
  ...
  files?: Array<{ id: string; name: string; size: number; type: string; content: string }>
  modelId?: string
  /** 'web-llm' for local inference, 'openai' for remote API, 'ollama' for local Ollama, 'lmstudio' for LM Studio, 'llamacpp' for llama.cpp, 'vllm' for vLLM, 'deepseek' for DeepSeek cloud API. null defaults to web-llm */
...
export async function* streamChatWithProvider(
  ...
  if (provider === 'deepseek') {
    // DeepSeek cloud API path
    const ds = options.deepSeekSettings
    if (!ds) {
      throw new Error('DeepSeek API settings not configured')
    }
    const modelId = ds.modelId ?? 'deepseek-chat'
    const { temperature = 0.7, top_p: topP = 0.9, maxTokens = 1024, systemPrompt } = options
